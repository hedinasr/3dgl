...
//! Add transparency to an image: all white pixels (R=255,G=255,B=255) get
//! transparent
void imCreateTransparency(Image &im, const unsigned char R = 255,
                          const unsigned char G = 255,
                          const unsigned char B = 255, int option = 0);
...
//! return the color value of a pixel (x,y); c=0 return R; c=1 return G; c=2
//! return B; c=4 return Alpha
unsigned char &getPix(const Image &im, const int x, const int y,
                      const int c = 0);

//! return the color value of a pixel (x,y); c=0 return R; c=1 return G; c=2
//! return B; c=4 return Alpha
const unsigned char &getPixConst(const Image &im, const int x, const int y,
                                 const int c = 0);

//! return the color value of a pixel (x,y); c=0 return R; c=1 return G; c=2
//! return B; c=4 return Alpha
void setPix(const Image &im, const int x, const int y, const int c,
            unsigned char v);
