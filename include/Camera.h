...
//! A camera is defined by a position and 2 angles of rotations (defining the
//! orientation)
struct Camera {
  Vec3f pos;
  float rotLeft;
...
inline void camInit(Camera &cam,
                    const Camera_FlyTrackball ft = CAMERA_TRACKBALL) {
  cam.pos.x = 0;
  cam.pos.y = 2;
  cam.pos.z = 10;
  cam.rotUp = 0;
...
inline void camTranslate(Camera &cam, const DIR where, const float c = 1.f) {
  ...
  if (vecLength(res) > 1.f)
    cam.pos = res;
}

enum CAMERA_MODE { CAMERA_STOP, CAMERA_ROTATION, CAMERA_TRANSLATION };
inline void camStartMove(Camera &cam, const CAMERA_MODE mode, const int x,
                         const int y) {
  cam.mode = mode;
  cam.mx = x;
  cam.my = y;
