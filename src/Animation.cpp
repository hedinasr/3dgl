...
const int NBPB = 10; // NomBre de Points de Bezier (entre 2 points de Contr il y
                     // aura NPB points)

float temps() { return float(clock()) / CLOCKS_PER_SEC; }
...
*/
Vec3f Bezier(const Vec3f &p0, const Vec3f &p1, const Vec3f &p2, const Vec3f &p3,
             const float t) {
  float t1, t13, t3;
  Vec3f p, r0, r1, r2, r3;
  t1 = 1 - t;
...
void animInit(Animation &a, const char *nom_fichier) {
  ...
  // donnée temporaire (=point de controle PC) servant pour calculer les points
  // finaux (P)
  int NBPC;
  Vec3f *PC;
  ...
  // Courbe de Bezier
  int c = 0, j;
  for (i = 0; i < NBPC; i += 4) // Pour calculer une courbe de Bezier, on
                                // regroupe les points de controle par 4
  {
    for (j = 0; j < NBPB; j++) {
      a.P[c] = Bezier(PC[i], PC[(i + 1) % NBPC], PC[(i + 2) % NBPC],
                      PC[(i + 3) % NBPC], float(j) / (NBPB));
      c++;
    }
  }
  delete[] PC;
